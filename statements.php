    ...
    <?php 
      ...
      if ($array ->num_rows > 0) 
      {
         while ($row = $array->fetch_assoc()) 
         {
            ...
            {
              echo "<div class='list-group-item alert alert-secondary'>You withdraw Tk.$row[debit] from your account at $row[date]</div>";
            }
            if ($row['action'] == 'deposit') 
            {
              echo "<div class='list-group-item alert alert-success'>You deposit Tk.$row[credit] in your account at $row[date]</div>";
            }
            if ($row['action'] == 'deduction') 
            {
              echo "<div class='list-group-item alert alert-danger'>Deduction have been made for  Tk.$row[debit] from your account at $row[date] in case of $row[other]</div>";
            }
            if ($row['action'] == 'transfer') 
            {
              echo "<div class='list-group-item alert alert-warning'>Transfer have been made for  Tk.$row[debit] from your account at $row[date] in  account no.$row[other]</div>";
            }
